...
def create_fat12_floppy(image_path, source_file):
    ...
    # Read the source file to inject
    ...
    struct.pack_into('<HHHI', root_dir, entry_offset + 22,
        0, 0, start_cluster, file_size) # Time, Date, Start Cluster, File Size
    ...
    data_area[0:file_size] = file_data
    ...
    print(f"Injected {filename} ({file_size} bytes) at cluster {start_cluster}")

if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: make_fat.py <output.img> <source_file>")
        sys.exit(1)
